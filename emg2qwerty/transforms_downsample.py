...
class Downsample:
    ...
    Args:
        ...
        For example, factor=2 converts 2kHz to 1kHz.
    """
    factor: int = 1

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        # If factor <= 1, do not apply downsampling
        if self.factor <= 1:
            return tensor

        # Assume tensor shape is (T, B, C)
        # T: time steps, B: batch size, C: channels
        # Select every `factor` step along the time dimension
