...
def main(config: DictConfig):
    ...
    # Experiment parameters: factor=1 (2kHz), 2 (1kHz), 3 (666Hz), 4 (500Hz/break)
    sampling_factor = 1
    
    from emg2qwerty.transforms import Downsample, Compose

    # Helper function to inject downsampling into the existing transform pipeline
    def wrap_with_downsample(base_transform, factor):
        if factor <= 1:
            return base_transform       
        return Compose([base_transform, Downsample(factor=factor)])    
    
    log.info(f"Instantiating LightningDataModule {config.datamodule} with Downsample factor {sampling_factor}")
    datamodule = instantiate(
        ...
        # Apply the wrapped transforms with the specified downsampling factor
        train_transform=wrap_with_downsample(_build_transform(config.transforms.train), sampling_factor),
        val_transform=wrap_with_downsample(_build_transform(config.transforms.val), sampling_factor),
        test_transform=wrap_with_downsample(_build_transform(config.transforms.test), sampling_factor),
        _convert_="object",
    )
