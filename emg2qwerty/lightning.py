...
class TransformerCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        # Model Pipeline
        self.model = nn.Sequential(
            # 1. Normalize the spectrograms (T, N, bands=2, C=16, freq)
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            # 2. Flatten spatial/freq dimensions: -> (T, N, 1056)
            nn.Flatten(start_dim=2),
            # 3. Our custom Encoder-only Transformer
            EMGTransformer(
    ...
    def _step(
        ...
        # Our Transformer does not perform temporal downsampling, so T_diff is 0
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(
            nn.Linear(lstm_out, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
...
class RNNCTCModule(pl.LightningModule):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        feats = self.front_end(inputs)
        h_seq, _ = self.rnn(feats)
...
# ----------------------------
class HybridTDSConvBiLSTMCTCModule(pl.LightningModule):
    """
    Hybrid architecture:
    SpectrogramNorm -> MultiBandRotationInvariantMLP -> Flatten
    -> TDSConvEncoder -> BiLSTM -> Linear -> LogSoftmax -> CTC
    """
    ...
    def __init__(
        ...
        # bidirectional=True => output dim = 2 * hidden_size
        self.classifier = nn.Sequential(
    ...
    def _step(
        ...
        # adjust lengths if encoder changes T
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
